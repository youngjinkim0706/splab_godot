...
struct TrieIteratorInternal {
	...
	TrieIteratorInternal(const NodeT *a_root) :
			m_root(a_root){};
	...
	bool step_down() {
		...
		while (it != x->end() and NodeT::value(it) == nullptr) {
			++it;
		}
	...
	bool step_fore() {
		...
		if (up != nullptr) {
			do {
				++m_ptrs.back();
			} while (m_ptrs.back() != up->end() and NodeT::value(m_ptrs.back()) == nullptr);
	...
	public:
		...
		iterator clone() {
			return (_impl.get() == nullptr) ?
						   iterator() :
						   iterator(new IteratorInternalT(*_impl));
		}

		bool operator==(const iterator &other) const {
			return _impl.get() == other._impl.get() || (_impl.get() && other._impl.get() && _impl->get() == other._impl->get());
		}
...
private:
	...
	iterator find_prefix_int(NodeT *root_node, KeyIterator it, KeyIterator kend, CallbackType exactMatch) {
		...
		general_search(
				...
				[&inputEnd](NodeItr, KeyIterator iend) { inputEnd = iend; });
...
public:
	...
	iterator find(KeyIterator it, KeyIterator kend) {
		...
		general_search(
				...
				[&output](NodeT *n, key_iterator, KeyIterator) { output.reset(); },
	...
	struct _debug_print {
		...
		_debug_print(const trie_map &amap) :
				map(amap){};
...
public:
	CStrIterator(AtomT *a_str) :
			m_str(a_str) {}
	explicit CStrIterator(AtomT *a_str, size_t offset) :
			m_str(a_str + offset) {}
